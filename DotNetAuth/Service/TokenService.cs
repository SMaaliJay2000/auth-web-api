...
namespace DotNetAuth.Service
{
    public class TokenService : ITokenService
    {
        ...
        {
            var signingCreedentials = new SigningCredentials(_secretKey, SecurityAlgorithms.HmacSha256);
            var claims = await GetClaimsAsync(user);
            var tokenOptions = GenerateTokenOptions(signingCreedentials, claims);
